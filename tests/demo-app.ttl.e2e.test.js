...
withTags(['ttl', 'tenant', 'control', 'standard', 'bearer'], () => {
  describe('RDCP Demo App - Temporary controls (TTL)', () => {
    ...
    const tokenCtrl = jwt.sign(
      {
        sub: 'ops@example.com',
        scopes: ['control', `control:${tenant}`, 'read', `read:${tenant}`],
      },
      secret,
    ...
    test('enable category with TTL expires after duration', async () => {
      const resEnable = await request(app)
        ...
        .send({
          action: 'enable',
          categories: ['CACHE'],
          options: { temporary: true, duration: '150ms' },
        })
    ...
    test('disabling cancels pending TTL for that category', async () => {
      ...
      await request(app)
        ...
        .send({
          ...
          options: { temporary: true, duration: '500ms' },
        })
        .expect(200)
    ...
    test('multiple categories get independent TTLs', async () => {
      await request(app)
        ...
        .send({
          ...
          options: { temporary: true, duration: '120ms' },
        })
        .expect(200)
      ...
      let res = await request(app)
        .get(`/rdcp/v1/tenants/${tenant}/settings`)
        .set(h('ttl8'))
      expect(res.body?.settings?.categories || []).toEqual(
        expect.arrayContaining(['AUTH', 'INTEGRATIONS'])
      )
      ...
      res = await request(app)
        .get(`/rdcp/v1/tenants/${tenant}/settings`)
        .set(h('ttl9'))
      expect(res.body?.settings?.categories || []).not.toEqual(
        expect.arrayContaining(['AUTH', 'INTEGRATIONS'])
      )
    })
    ...
    test('TTL applies only when options.temporary is true and duration is valid', async () => {
      await request(app)
        ...
        .send({
          action: 'enable',
          categories: ['REPORTS'],
          options: { temporary: false, duration: '100ms' },
        })
        .expect(200)
    ...
    test('invalid or zero durations do not schedule TTL', async () => {
      await request(app)
        ...
        .send({
          action: 'enable',
          categories: ['QUERIES'],
          options: { temporary: true, duration: 'xyz' },
        })
        .expect(200)
