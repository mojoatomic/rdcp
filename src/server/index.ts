...
interface RDCPStatusResponse extends RDCPResponse {
  ...
  performance: {
    ...
    metrics?: {
      cpu?: { value: number; unit: 'percent'; measured: true }
      memory?: { value: number; unit: 'bytes'; measured: true }
      eventLoopDelayP99?: { value: number; unit: 'milliseconds'; measured: true }
    }
...
export class RDCPServer {
  ...
  handleStatus(
    ...
    const memStr = `${(rssBytes / (1024 * 1024)).toFixed(1)}MB`
    ...
    const response = {
      ...
      performance: {
        ...
        metrics: {
          cpu: { value: Number(cpuPercent.toFixed(2)), unit: 'percent' as const, measured: true as const },
          memory: { value: rssBytes, unit: 'bytes' as const, measured: true as const },
          ...(p99DelayMs !== undefined
            ? { eventLoopDelayP99: { value: Number(p99DelayMs.toFixed(2)), unit: 'milliseconds' as const, measured: true as const } }
            : {}),
        },
